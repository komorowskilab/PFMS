...
void pathFinder(const int count, const vector <int> &order, const vector <double> &data, const int total, 
		int *&path, double *&proba, int *&hits, const double totalMapReads, const double totalPeakReads, 
		const double totalPeakArea,const double medianPeakBinCount, const int numPeaks,const int readcoverage);
...
{
	int j,numCycles=1; 
	int count,total,windowSize;
	int readcoverage;//added 04/13/08
	vector <int> order;
	vector <double> data;
	int *path;
	int *hits;
	...
	ofstream outPutFile;
	double totalMapReads;
        double totalPeakReads;
        double totalPeakArea;
        double medianPeakWidth;
	double medianPeakBinCount;
        int numPeaks;
        istringstream iss;
    	string lineString;   
	double temval;
	...
	if(medianPeakWidth >800)
		medianPeakWidth = 800;
	medianPeakBinCount = medianPeakWidth /(double)windowSize; 
	...
	{
		pathFinder(count,order,data,total,path,proba,hits,totalMapReads,totalPeakReads,totalPeakArea,medianPeakBinCount,numPeaks,readcoverage);
	}
