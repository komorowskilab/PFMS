# Time-stamp: <2009-09-28 17:26:15 Tao Liu>
...
class PeakModel:
    """Peak Model class.
    """
    def __init__ (self, opt=None, treatment=None, max_pairnum=500, gz = 0, fold=32, bw=200, ts = 25, bg=0, max_dup_tags=1):
        self.treatment = treatment
        if opt:
            self.gz = opt.gsize
            self.fold = opt.mfold
            self.tsize = opt.tsize
            self.bg_redundant_rate = opt.bg_redundant
            self.mr = opt.max_dup_tags
            self.bw = opt.bw
            self.info  = opt.info
    ...
    def build (self):
        ...
        self.min_tags = float(self.treatment.total_unique) * self.fold * self.peaksize / self.gz /2 # mininum unique hits on single strand
        ...
        if num_paired_peakpos < 100:
            self.error("Too few paired peaks (%d) so I can not build the model! Lower your MFOLD parameter may erase this error." % (num_paired_peakpos))
            self.error("Process is terminated!")
            sys.exit(1)
...
Summary of Peak Model:
  ...
  Fragment size: %d
  Scan window size: %d
""" % (self.min_tags,self.d,self.scan_window)
