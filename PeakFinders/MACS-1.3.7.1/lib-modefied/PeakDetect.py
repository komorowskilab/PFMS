# Time-stamp: <2009-10-15 14:54:34 Tao Liu>
...
from OutputWriter import zwig_write
from IO import PeakIO,WigTrackI
from IO.BinKeeper import BinKeeperI
from Prob import poisson_cdf,poisson_cdf_inv
...
class PeakDetect:
    ...
    def __call_peaks_w_control (self):
        ...
        if self.save_wig:
            ...
            if self.opt.wigextend:
                zwig_write(self.treat,self.opt.wig_dir_tr,self.zwig_tr,self.opt.wigextend,log=self.info,space=self.opt.space)
            else:
                zwig_write(self.treat,self.opt.wig_dir_tr,self.zwig_tr,self.d,log=self.info,space=self.opt.space)
        ...
        if self.save_wig:
            ...
            if self.opt.wigextend:
                zwig_write(self.control,self.opt.wig_dir_ctl,self.zwig_ctl,self.opt.wigextend,log=self.info,space=self.opt.space)
            else:
                zwig_write(self.control,self.opt.wig_dir_ctl,self.zwig_ctl,self.d,log=self.info,space=self.opt.space)
    ...
    def __call_peaks_wo_control (self):
        ...
        self.final_peaks = self.__filter_w_control(peak_candidates,self.treat,self.treat,1,pass_1k=True)
    ...
    def __build_wigtrackI (self, trackI, space=10):
        ...
        for chrom in chrs:
            tags = trackI.get_ranges_by_chr(chrom)[0]
            l = len(tags)
    ...
    def __diag_peakfinding_wo_control_sample (self, percent):
        ...
        final_peaks_percent = self.__filter_w_control(peak_candidates,self.treat,self.treat,1,pass_1k=True) # bug fixed...
        return final_peaks_percent
