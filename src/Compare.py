...
def Sort_numbers(numbers, file_numbers, markers):
    ...
    return numbers, file_numbers, markers
        

def Compare_results(label, files, min_rank):
    results_files_id = []
...
#sort based on the first array and change value of other arrays accordingly
    Begin_End_array, Begin_End_file_numbers_array, Begin_End_array_marker = Sort_numbers(Begin_End_array, Begin_End_file_numbers_array,Begin_End_array_marker)
...
#Wpisuje dane do pliku tymczasowego                     
        temp_compare_id.write(operation_number + "\t" + operation_marker + "\t" + str(k) + "\n")
...
#at least half of the peakfinders should has found it
    ...
    while temp_line != "":
        ...
        if splited_line[1] == "start" and (splited_line[2]).isdigit():
            if int(splited_line[2]) > max_number_of_votes:
                max_number_of_votes = int(splited_line[2])
            if int(splited_line[2]) == min_rank: #default value is: (math.floor(len(peakfinders)/2-0.1) + 1)
                output_file_id.write(chromosome + "\t" + splited_line[0] + "\t")
#            print(chromosome + "\t" + splited_line[0] + "\t")
        elif splited_line[1] == "stop" and splited_line[2].isdigit():
            if int(splited_line[2][0:1]) == min_rank - 1: #default value is: math.floor(number_of_Peak_Finder_files/2-0.1)
                output_file_id.write(splited_line[0] + '\t' + "PFMS_peak_" + str(peak_number) + '\t' + str(max_number_of_votes) + "\n")
